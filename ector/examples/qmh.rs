...
async fn main(spawner: embassy_executor::Spawner) -> ! {
    ...
    let inbox_1 = ActorQMH::spawn(spawner, config).expect("ActorQMH failed to start");
...
/** Public Elements of the Actor Below */
        ...
        loop {
            let deadline = async {
                match self.scheduler.as_mut() {
                    Some(Scheduler { timer: next_timer, .. }) => next_timer.await,
                    None => pending().await,
                }
            };
...
impl ActorQMH {
    /// Create a new actor with a spawner and a configuration.
    fn new(_spawner: embassy_executor::Spawner, config: Config, _inbox: ActorInbox<Message>) -> Self {
...
async fn actor_task(context: &'static ActorContext<ActorQMH>, actor: ActorQMH) {
